...
class _SfMessageChatScreenState extends State<SfMessageChatScreen> {
  ...
  Future<void> isCallAvailable() async {
    try {
      ...
      List availableModule = modulesList.map((e) => e['name'].toString()).toList();
  ...
  Widget build(BuildContext context) {
    ...
    return Consumer<ChatMessageController>(builder: (context, ref, child) {
      ...
      if (ref.chatHistoryList.isNotEmpty) {
        ...
        debugPrint("📊 First message ID: ${ref.chatHistoryList.first.messageId}");
      }
  ...
  Widget _pageBody() {
    return Consumer<ChatMessageController>(
      builder: (context, ref, child) {
        // ✅ Debug info
        debugPrint("🔄 Building page body - Chat count: ${ref.chatHistoryList.length}");
  ...
  void _onClearChat() {
    ChatMessageController messageController = Provider.of(context, listen: false);
    DashBoardController dbController = Provider.of(context, listen: false);
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          if (!showDateLabel && index > 0) {
            ...
            if (prevItem.createdDate != null && prevItem.createdDate!.isNotEmpty) {
              try {
          ...
          List<ButtonItem> buttons = (item.button?.isNotEmpty ?? false)
  ...
  Widget _buildMessageInputArea() {
    return Padding(
      ...
      child: Consumer3<TemplateController, SfFileUploadController, ChatMessageController>(
        builder: (context, tempCtrl, fileUploadController, chatMsgController, child) {
          return Column(
            children: [
              const Divider(),
  ...
  Widget _buildSendButton(
    ...
    return InkWell(
      onTap: () async {
        ...
        if (msgController.text.isNotEmpty && chatMsgController.selectedFile == null) {
...
Future<void> pickImageFromGallery(context) async {
  final pickedFile = await FilePicker.platform.pickFiles(
    ...
    allowedExtensions: [
      "jpg", "jpeg", "png", "gif", "pdf", "html", "txt",
      "doc", "docx", "ppt", "pptx", "xls", "xlsx", "mp4",
      "mov", "avi", "mkv", "csv", "rtf", "odt", "zip", "rar",
    ],
  );
  if (pickedFile != null) {
    ChatMessageController chatMsgController =
